...
class ImagesController extends AppController {
    ...
    public function beforeFilter() {
        parent::beforeFilter();
        $this->fileFolder = APP . WEBROOT_DIR . DS . 'img' . DS . 'applications' . DS;
    ...
    public function admin_edit($id = null) {
        ...
        $applications = $this->Image->Application->find('list');
    ...
    private function saveFile() {
        if ($this->request->data['Image']['image']['error'] == 0) {
            $file = new File($this->request->data['Image']['image']['tmp_name']);
            $file->copy($this->fileFolder . $this->Image->id, true);
        }
    }
